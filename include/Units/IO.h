...
namespace Units
{
	namespace details
	{
		std::string generateExponent(int n)
		{
...
#undef PAIR_TYPE
		...
		std::string to_string_precision(double a_value, int n)
		{
			char data[10];
		...
		std::string magnitude_prefix(double qty)
		{
		...
		std::string unit_raw(Units::Unit un)
		{
		...
		std::string magnitude_scientific(double qty)
		{
			if(!std::isfinite(qty)) return format_inf(qty);
	...
	{
		using namespace details;
		if(u.eflag()) return "ERROR";

		std::string str;

		/**/ if(find_unit(u, str)) return str;
		else if(find_unit(std::sqrt(u   ), str)) return str + "²";
		else if(find_unit(std::sqrt(u^-1), str)) return str + "⁻²";
		else if(find_unit(std::cbrt(u   ), str)) return str + "³";
	...
	std::string to_string(Quantity q)
	{
		...
		/**/ if(find_unit(q.unit(), str)) return magnitude_prefix(q.magnitude()) + str;
		else if(find_unit(std::sqrt(q   ).unit(), str) && q.magnitude() > 0.0) return magnitude_prefix(q.magnitude()) + str + "²";
		else if(find_unit(std::sqrt(q^-1).unit(), str) && q.magnitude() > 0.0) return magnitude_prefix(q.magnitude()) + str + "⁻²";
		else if(find_unit(std::cbrt(q   ).unit(), str)) return magnitude_prefix(q.magnitude()) + str + "³";
		...
		return to_string_scientific(q);
	}
}

std::ostream& operator<<(std::ostream& os, Units::Quantity q) { return os << to_string(q); }
std::ostream& operator<<(std::ostream& os, Units::Unit u)     { return os << to_string(u); }
